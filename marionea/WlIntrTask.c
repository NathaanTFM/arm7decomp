...
{                                      // WlIntrTask.c:106
    ...
    {                            // :116
        ...
        if (((*(u16 *)((u32)&W_TXBUF_LOC1 + 4 * i) & 0x8000) != 0) || !pTxq->Busy)
        { // :120
            continue;
        }
        ...
        if (pTxq->pFrm)
        {
            ...
            if (pTxq->pMacFrm->Dot11Header.FrameCtrl.Data & 0x4000)
            { // :137
...
{                                                  // WlIntrTask.c:240
    ...
    for (u32 i = 0;; i++)
    { // variable present in older versions, probably still exists
...
{                                         // WlIntrTask.c:770
    ...
    if (W_LISTENCOUNT == 0 && pHeapMan->TxPri[2].Count)
    {                    // :782
        W_TXREQ_SET = 8; // :785
        TxqPri(2);       // :786
        return;
