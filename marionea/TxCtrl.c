...
{                                            // TxCtrl.c:298
    ...
    if (pTxFrm->Dot11Header.FrameCtrl.Data & 0x4000)
    { // WEP ?
...
{ // TxCtrl.c:936
    ...
    if (wlMan->TxCtrl.Txq[2].pMacFrm->Dot11Header.FrameCtrl.Bit.MoreData == 0)
    {                      // :973
        W_TXREQ_RESET = 8; // :975
        W_TXREQ_SET = 5;   // :976
...
{                                     // TxCtrl.c:2614
    ...
    for (i = 0; i < 0x10; i++)
    { // :2629
        if (pWork->RateSet.Support & (1 << i))
        { // :2631
            if (pWork->RateSet.Basic & (1 << i))
            {                                                        // :2633
