        ...
        case MODE_HOTSPOT:
            if ((pFrm->Dot11Header.FrameCtrl.Data & 1) == 0)
            {
                if (pWork->PowerMgtMode != 0 && (pFrm->Dot11Header.FrameCtrl.Data & 0x2000) == 0)
                {
                    ...
                    else if (pWork->Mode != MODE_HOTSPOT)
                    {
                        pWork->bExistTIM &= ~2;
                    }
...
{                                                 // RxCtrl.c:450
    ...
    if (pTxCtrl->Mp.Busy && MatchMacAdrs(pFrm->Dot11Header.Adrs1, wlMan->Work.BSSID) && pFrm->MacHeader.Rx.MPDU - 24 <= pMpKey->length - 8)
    {
...
{                                           // RxCtrl.c:740
    ...
    {                                                   // :763
        ...
        {                                                           // :772
            ...
            if (elementCheck.pCFP && (pFrm->Dot11Header.Duration & 0x8000) != 0)
            {                                                                                                                                       // :802
                W_CONTENTFREE = (WL_ReadByte(&elementCheck.pCFP->CFPDurRemain.u8[0]) + (WL_ReadByte(&elementCheck.pCFP->CFPDurRemain.u8[1]) << 8)); // :804
            }

            if (pMLME->State == STATE_SCAN_3 && pMLME->pReq.Scan->scanType == 1)
            { // :808
                if ((elementCheck.matchFlag & 9) == 9)
                {                                                         // :812
            ...
            {
                if ((elementCheck.matchFlag & 8) != 0)
                { // :821
                    if (pMLME->State == STATE_JOIN_1)
                    {                   // :830
                        ClearTimeOut(); // :833
                        if ((elementCheck.matchFlag & 0x30) != 0x30)
                        {                                           // :839
                            pMLME->Work.Scan.MaxConfirmLength = 12; // :841
                            pMLME->Work.Scan.ChannelCount = 10;     // :842
                        }
                        else if (pFrm->Body.BeaconInterval > 1000)
                        {                                           // :855
                        ...
                        else
                        {
                            ...
                            if ((elementCheck.foundFlag & 2) != 0 && (elementCheck.matchFlag & 2) == 0)
                            {                                         // :869
                    ...
                    case MODE_HOTSPOT:
                        ...
                        if (pWork->Mode == MODE_CHILD && pWork->bFirstTbtt)
                        { // :993
                            lltsf -= n;
                        ...
                        if (pWork->STA == STA_CLASS3 && elementCheck.pTIM && pWork->PowerMgtMode == 1)
                        {                                                             // :1035
                            ...
                            {                        // :1066
                                n = pWork->AID - n1; // :1069
                                if ((WL_ReadByte(&elementCheck.pTIM->VitrualBitmap[n >> 3]) & (1 << (n & 7))) != 0)
                                {                          // :1073
...
{                                           // RxCtrl.c:1228
    ...
    // holy
    if (
        (pAssReq->CapaInfo.Data & 0xFFC2) != 0 || (pAssReq->CapaInfo.Bit.ESS == 0) || (!pConfig->WepMode && pAssReq->CapaInfo.Bit.Privacy == 1) || (pConfig->WepMode && pAssReq->CapaInfo.Bit.Privacy == 0) || (pConfig->PreambleType == 1 && pAssReq->CapaInfo.Bit.ShortPreamble == 0))
    {
        stsCode = 10;
    }
    else
    {
        ...
        if ((elementCheck.matchFlag & 1) == 0)
        {                // :1328
            stsCode = 1; // :1331
        }
        else if ((elementCheck.matchFlag & 4) == 0)
        {                 // :1336
            stsCode = 18; // :1341
        }
        else
...
static void RxAssResFrame(ASSRES_FRAME *pFrm)
{
    ...
    if (pWork->Mode != MODE_CHILD && pWork->Mode != MODE_HOTSPOT)
        return;
...
static void RxReAssReqFrame(REASSREQ_FRAME *pFrm)
{
    ...
    // holy
    if (
        (pReAssReq->CapaInfo.Data & 0xFFC2) != 0 || (!pWlMan->Config.WepMode && pReAssReq->CapaInfo.Bit.Privacy == 1) || (pWlMan->Config.WepMode && pReAssReq->CapaInfo.Bit.Privacy == 0))
    {
        stsCode = 10;
    }
    else
...
{                                     // RxCtrl.c:1993
    ...
    if (wlMan->WlOperation & 8)
    {
        ...
        map = W_TXREQ_READ;
        if (
            ((map & 1) == 0 || !pTxq[0].Busy) && ((map & 4) == 0 || !pTxq[1].Busy) && ((map & 8) == 0 || !pTxq[2].Busy) && (W_RF_PINS & 1) == 0)
        {
            W_WEP_CNT = 0;
            W_WEP_CNT = 0x8000;
    ...
    if (!IsExistManFrame(pFrm->Dot11Header.SA, 0xB0))
    {
        ...
        {
        case 0:
            if (pWork->Mode == MODE_PARENT && wlMan->Config.AuthAlgo == 1)
            {
                stsCode = 13;
                bTxAuth = 1;
            }

            else if (pWork->Mode == MODE_PARENT)
            { // :2103
                bTxAuth = 1;
...
#define DEFRAG_MATCHES(tbl1, tbl2) \
    (MatchMacAdrs((tbl1)->DA, (tbl2)->DA) && MatchMacAdrs((tbl1)->SA, (tbl2)->SA) && (tbl1)->SeqCtrl.Bit.SeqNum == (tbl2)->SeqCtrl.Bit.SeqNum)
