...
{                                                 // MLME.c:166
    ...
    if (wlMan->Work.STA < STA_CLASS1)
        return 1; // :191
    if (pReq->ssidLength > 0x20)
        return 5; // :198
    if (pReq->scanType > 1)
        return 5; // :199
    if (!WL_ReadByte(pReq->channelList))
        return 5; // :200
    if (pReq->maxChannelTime > 0x3E8)
        return 5; // :201
    if (pReq->maxChannelTime < 0xA)
        return 5; // :202
    if (pReq->bssidMaskCount > 0x10)
        return 5; // :203
...
{                                     // MLME.c:245
    ...
    if (pWlMan->Config.Mode != 3 && pWlMan->Config.Mode != 2)
        return 11;
    if (pWork->STA < STA_CLASS1)
        return 1;
    WSetStaState(STA_CLASS1);
    if ((pReq->bssDesc.bssid[0] & 1) != 0)
        return 5;
    if (pReq->bssDesc.ssidLength == 0)
        return 5;
    if (pReq->bssDesc.ssidLength > 32)
        return 5;
    if (pReq->bssDesc.beaconPeriod < 10)
        return 5;
    if (pReq->bssDesc.beaconPeriod > 1000)
        return 5;
    if (pReq->bssDesc.dtimPeriod > 0xFF)
        return 5;
    if ((pReq->bssDesc.channel & 0xFFF0) != 0)
        return 5;
    if (!CheckEnableChannel(pReq->bssDesc.channel))
        return 5;
    if ((pReq->bssDesc.rateSet.basic & (u32)(-0x1000)) != 0)
        return 5; // WHAT?
    if ((pReq->bssDesc.rateSet.support & (u32)(-0x1000)) != 0)
        return 5; // WHAT?
    if (pReq->bssDesc.rateSet.basic == 0)
        return 5;
    if ((pReq->bssDesc.rateSet.support | pReq->bssDesc.rateSet.basic) == 0)
        return 5; // makes the cond above useless
    if (pReq->timeOut > 2000)
        return 5;
...
{                                                   // MLME.c:566
    ...
    if (pWork->STA < STA_CLASS2)
        return 1;
    if (pReq->newApMacAdrs[0] & 1)
        return 5;
    if (pReq->listenInterval < 1)
        return 5;
    if (pReq->listenInterval > 0xFF)
        return 5;
    if (pReq->timeOut > 2000)
...
{                                                   // MLME.c:719
    ...
    if (pReq->ssidLength == 0)
        return 5;
    if (pReq->beaconPeriod < 10)
        return 5;
    if (pReq->beaconPeriod > 1000)
        return 5;
    if (pReq->dtimPeriod == 0)
        return 5;
    if (pReq->dtimPeriod > 0xFF)
        return 5;
    if ((pReq->channel & 0xFFF0) != 0)
        return 5;
    if (!CheckEnableChannel(pReq->channel))
        return 5;
    if (pReq->basicRateSet == 0)
        return 5;
    if ((pReq->basicRateSet & (u32)(-0x1000)) != 0)
        return 5; // WHAT?
    if (pReq->supportRateSet == 0)
        return 5;
    if ((pReq->supportRateSet & (u32)(-0x1000)) != 0)
        return 5; // WHAT?
    if (pReq->gameInfoLength > 0x80)
        return 5;
    if (FLASH_VerifyCheckSum(0))
        return 14;
...
{                                                         // MLME.c:803
    ...
    if (pReq->ccaMode > 3)
        return 5;
    if (pReq->edThreshold > 0x3F)
        return 5;
    if (pReq->measureTime == 0)
        return 5;
    if (pReq->measureTime > 1000)
        return 5;
