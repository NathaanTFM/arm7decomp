...
u16 WMSP_GetLinkLevel(u32 rssi) { // wmsp_system.c:715
    if ( wmspW.wm7buf->connectPInfo.gameInfo.platform == 8 ) {
        if (rssi < 0x16) return 0;
        if (rssi < 0x1C) return 1;
        if (rssi < 0x22) return 2;
        return 3;
        
    } else {
        if (rssi < 0x8) return 0;
        if (rssi < 0xE) return 1;
        if (rssi < 0x14) return 2;
        return 3;
    }
}
